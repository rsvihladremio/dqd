...
public class Human {
  ...
  /**
   * Convert into a string the appropriate time unit for a time span measured in
   * milliseconds. The
   ...
   * @return a string formatted in an easily readable string, the max unit of
   *         measure is days out in
   *         days
   */
  ...
  /**
   * Convert bytes into a string measurement of terabytes, gigabytes, megabytes,
   * kilobytes or bytes.
   * Appropriate for RAM, but some would argue not for disk size or network
   * bandwidth. For file size
   * this is largely a matter of preference
   *
   * @see <a href="https://wiki.ubuntu.com/UnitsPolicy">Ubuntu Units Policy</a>
   * @param bytes number of bytes
   * @return formatted string with decimal precision of 2, the largest unit of
   *         measure will be
   *         terabytes
   */
